...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleClientValidationException(ClientValidationException ex) {
        log.error("Client validation error: {}", ex.getMessage());
        
        Map<String, Object> error = new HashMap<>();
        error.put("timestamp", LocalDateTime.now());
        error.put("status", HttpStatus.BAD_REQUEST.value());
        error.put("error", "Client Validation Failed");
        error.put("message", ex.getMessage());
        error.put("path", "/api/orders");
    ...
    public ResponseEntity<Map<String, Object>> handleCartEmptyException(CartEmptyException ex) {
        log.error("Cart empty error: {}", ex.getMessage());
        
        Map<String, Object> error = new HashMap<>();
        error.put("timestamp", LocalDateTime.now());
        error.put("status", HttpStatus.BAD_REQUEST.value());
        error.put("error", "Cart Empty");
        error.put("message", ex.getMessage());
        error.put("path", "/api/orders");
    ...
    public ResponseEntity<Map<String, Object>> handleCheckoutValidationException(CheckoutValidationException ex) {
        log.error("Checkout validation error: {}", ex.getMessage());
    ...
    public ResponseEntity<Map<String, Object>> handleGenericException(Exception ex) {
        ...
        error.put("message", "An unexpected error occurred during order processing");
        error.put("path", "/api/orders");
